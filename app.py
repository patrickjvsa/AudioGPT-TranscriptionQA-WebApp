...
if __name__ == "__main__":
    ...
    st.subheader('audioGPT by patrick🤖')
    load_dotenv()
    ...
    with st.sidebar:
        ...
        st.write('Para obtener una api key, visita https://platform.openai.com/account/api-keys')
        ...
        uploaded_file = st.file_uploader('Subir un archivo nuevo:', type=['m4a', 'mp3','mp4', 'mpeg', 'mpga', 'wav', 'webm'])
        # boton de añadir
        ...
        if uploaded_file and add_data_button:
            with st.spinner('Procesando...'):
                ...
                chunks = tm.ChunkData.chunk_data(data=data)
    ...
    # transcripción
    st.text_area('Transcripción: ', value=data, height=300)
    ...
    # query del usuario
    query = st.text_area('Inserte pregunta sobre el audio...', height=160)
    ...
    if query and is_button_pressed:
        ...
        if 'vs' in st.session_state:
            with st.spinner('Pensando...'):
                ...
                answer = tm.SimpleQuestionAnswer.ask_and_get_answer(query=query, vector_store=vector_store, llm=chatbot, k=k)
